...
export const TagFilterComponent: React.FC<{ tags: string[], tagsChange: (selectedCategory: string) => void }> = ({ tags, tagsChange }) => {
  const [currentPage, setCurrentPage] = useState(tags[0]); // Initial state
    ...
    <div className="flex items-center justify-center py-4 md:py-8 flex-wrap">
      {tags.map((tag) => (        
        <button
          ...
          aria-current={currentPage === tag ? 'page' : undefined}>
          {tag}
        </button>
      ))}
    </div>
